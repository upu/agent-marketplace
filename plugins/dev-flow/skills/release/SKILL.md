...
12. **リリース結果検証** — `gh run list --workflow=release.yml -L 1` で対象 run を取得し（直前 merge が最新 main push のため通常これが対象）、`gh run watch <run-id> --exit-status` をパイプせず単発フォアグラウンドで直接実行する（長めタイムアウト例: 600000 ms）。Windows PowerShell・POSIXシェルのどちらでも `--exit-status` の終了コードがそのままコマンドの終了コードになり、non-zero を確実に検知できる（出力の全表再出力を `tail` で削る移植性より、終了コード保持を優先する）。green なら GitHub Release（副作用として `vx.y.z` タグ生成）+ `.vsix` 添付が成功し、`VSCE_PAT` があれば Marketplace 公開も実行済み。
