...
10. **（要求された場合）Copilot レビュー待機**
  - このリポジトリの branch ruleset は push ごとの Copilot 自動レビュー要求（`copilot_code_review`, `review_on_push: true`）を設定できるが、常時有効とは限らない。PR ごとに検知し、確認対象は必ず現 HEAD の commit sha に限定する（古い commit のレビューを新 push の完了と誤認しないため）。
    sha の取得とレビュー状態の確認は2回の Bash 呼び出しに分ける: まず `git rev-parse HEAD` 単体で sha を取得し、次に別呼び出しで `gh pr view <pr> --json reviewRequests,reviews` を実行する（`--jq` は使わない）。`gh pr view` の `--jq` は jq 式1つのみを受け付け、`jq` 本体のような `--arg` による変数注入をサポートしないため、`--jq --arg sha "$sha" '...'` のような形は `accepts at most 1 arg(s), received N` のエラーで必ず失敗する——返ってきた生の JSON を自分で読み、`reviewRequests[].login` に `Copilot` があるか、`reviews[]` に `author.login == "copilot-pull-request-reviewer"` かつ `commit.oid` が先の sha と一致するエントリがあるかを確認する。
    ここで login 文字列は状態で異なる。*依頼中* は `reviewRequests` に `Copilot`、*提出済みレビュー* は `author.login` が `copilot-pull-request-reviewer`。片方だけ見て判断しない。**短時間で空だから未設定と判断しないこと**。until-loop（チェーンリトライではない）で約 20-30 秒間隔、合計 10-15 分程度監視し、各回でこの `gh pr view` 呼び出しをやり直して `reviewRequests` に `Copilot` があるか、`reviews` に該当 sha のエントリがあるかを確認する（sha は最初に取得した値を使い回し、新しい push があった時だけ取り直す）。全期間を通して両方空のときのみ「この commit では Copilot レビュー適用なし」と結論し Merge へ進む。途中でどちらかが出たら、該当 sha のレビューが `reviews` に現れるまで待機(または既に現れているなら内容読解へ進む)。
