...
## 手順1: 操作感に影響するissueで実装前に操作モデルを合意する理由

upu/Totonoe-Log v0.8.0（17 issue／5日間）で、同一リリース内に「出したばかりの操作を作り直す」issueが3件発生した実測:

| 元 | 作り直し | 何が変わったか |
| --- | --- | --- |
| #179 | #191 | 行ジャンプ: シングルクリック → ダブルクリック＋右クリックメニュー（クリックだとテキスト選択を邪魔する） |
| #180 | #194 | マスク: ワンショットの「マスクしてコピー」ボタン → 可逆な表示状態のトグル |
| #195 | #212 | マスク対象の指定: 正規表現のみ → キー名を並べるだけの構造化された欄を追加 |

いずれも実装・レビュー・マージまで通した後に「触ってみて違う」と分かったもので、CIや自動テストでは検出できない種類の手戻り（#180→#194は未リリースのボタンを差し替える設計変更、#195→#212は同日中の追加対応だった）。3件に共通するのは、実装前に決めておけば避けられた3つの論点——(1) 可逆なトグルか戻せないワンショットか（#194の教訓）、(2) 非専門家が到達できるか（#212の教訓、正規表現の読み書きを前提にしないこと）、(3) 既存のどの操作と役割が重なるか（#191の教訓、クリックはテキスト選択と競合する）。

同じ教訓は既にメモリに2件記録されていた（「パターン入力のUXは手戻りしやすい」「Interactive View は操作より表示状態」）が、それでも3回目が起きた。想起頼みでは再発を防げないと判断し、実装前に必ず通る手順1のサブ項目として固定した（agent-marketplace#104）。
