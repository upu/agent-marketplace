...
14. **報告する** — マージされたPR番号、`Closes #N` によりissueが自動クローズされたこと、新しい `main` のコミット、Copilotレビューの結果（あれば）、掃除したローカルブランチを述べる。手順12を実施した場合は、実発火確認の結果（成功/失敗、runへのリンク）も併せて述べる。
    - **手動確認の提案**: 変更がユーザーの操作感に影響する場合（新しいUI・コマンド・オプションの追加、操作手順や入出力の見た目の変更、対話フローの変更）、報告の末尾で人間による手動確認を提案する。提案には必ず次の2点をセットで書く: (1) **試し方**——前提条件（インストール・再読み込み・プラグイン更新等）込みの、そのまま実行できる具体的なコマンド・操作手順。(2) **OKの基準**——何がどう見えれば/動けば成功か、期待される出力・挙動。確認で問題が見つかった場合はfollow-up issueを起票する旨も添える。内部リファクタ・CI・テストのみ等、操作感に影響しない変更では提案不要。
