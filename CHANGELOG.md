...
- `dev-flow`'s `fetch-copilot-feedback.js` now scopes `inlineComments` to the matched review's own comments instead of returning every `Copilot`-authored inline comment on the PR. Previously, once a PR had been reviewed by Copilot more than once, still-open inline comments from earlier reviews on older commits bled into the latest review's `inlineComments`, even when that review's body said it added no new comments (as seen on PR #5, where the newest review's "no new comments" summary still came back with 4 stale comments attached). Comments are now matched to the target review via the REST API's numeric review id (`pull_request_review_id`), resolved from `gh api repos/:owner/:repo/pulls/<pr>/reviews` since `gh pr view --json reviews` only exposes a non-numeric GraphQL node id; `summary`/`state` are unaffected.
- `dev-flow`'s `compile-if-ts.js` PostToolUse hook now also runs `npm run compile` for `.tsx`/`.mts`/`.cts` edits and for `tsconfig.json`/`tsconfig.*.json` edits, instead of only `.ts`. `shouldCompile` previously matched a plain `\.ts$` suffix, so editing a React component, an ESM/CJS dual-module entry point, or a `tsconfig` that changes compile output left the hook silently doing nothing — the compile guard looked active but missed exactly the files a mixed TypeScript project touches most. Per issue #99's design memo, the check is now an explicit allowlist (extensions `.ts`/`.tsx`/`.mts`/`.cts`, plus `tsconfig.json`/`tsconfig.*.json` by file name) rather than excluding known-irrelevant files, so an unrecognized extension still fails safe by skipping, same as before. `package.json` stays out of the allowlist (it doesn't change compile output by itself); the missing-path fallback and `.ts` behavior are unchanged.
